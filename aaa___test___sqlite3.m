function aaa___test___sqlite3(varargin)
%test by executing the examples
clc

if ifversion('<',7,'octave','<',0)% suppress v6.5 warning
    warning off MATLAB:m_warning_end_without_block;clc
end

bdname=tmpname('HJW_test_sqlite3_','.db');
try
    % creating a table
    sqlite3(bdname,'CREATE TABLE test (some_text TEXT, some_int INT, some_real REAL);');
...
%convert int64 to double to eliminate the difference between Matlab and Octave
for n=1:numel(y)
    y(n).some_int=double(y(n).some_int);
end
y=y(:);%Matlab returns 4x1, Octave returns 1x4
z=rmfield(y,'real');%for CLI-cases the real field causes errors due to rounding

if max(abs([y.real]-[2.71828 9 2*pi 3.14159]))>=1e4
    error('output incorrect')
end
if ifversion('<',7,'octave','<',0) || ...
        ( ifversion('==','R2011a','octave','<',0) && isunix && ~ismac )
    %fake the results of this specific test for CLI cases
    y_hash=ComputeNonCryptHash(y,256);
else
    y_hash='FB66BA89EDD4F680A8CE199CF8D6B27A5472C494B336FFB9D2A6D80D806ACF00';
end
y_hash2=ComputeNonCryptHash(y,256);
z_hash='6C4E5A42C2E4A292F5DA28AA56E8F2E2BFE760B78FFB355F06C63B9C59F56B39';
z_hash2=ComputeNonCryptHash(z,256);
if ~strcmp(y_hash,y_hash2) || ~strcmp(z_hash,z_hash2)
    error('output incorrect')
else
...
data=shuffle_uint16(data);
data=uint16_to_logical(data);
...
hash=dec2hex_(data);hash=reshape(hash.',1,[]);
end
function hash=dec2hex_(data)
...
function data=shuffle_uint16(data)
...
function data=shuffle_uint16_inv(data)
base=65537;%base=(1+(2^16));
...
function data=uint16_to_logical(data)
...
function data=logical_to_uint16(data)
